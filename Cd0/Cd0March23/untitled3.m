...
%
% Grid convention:
%   row index    -> altitude index
%   column index -> Mach index
%
% So:
%   cd0_total_grid(j,k) = C_D0 at alt(j), mach(k)
% this is how Jordan will reference it in her code
...
sref      = Aircraft.wing.S_ref;        % reference area [ft^2]
...
comp(2).f    = comp(2).l / comp(2).d;       % fineness ratio, l/d [-]
...
for j = 1:nAlt
    ...
    % atmosisa inputs/outputs use SI
    [T, a, P, rho] = atmosisa(h_m); 
    mu = sutherland_mu(T);          % dynamic viscosity [kg/(m*s)]

    for k = 1:nMach

        M = mach(k);                % Mach number [-]
        V = M * a;                  % flight speed [m/s]

        cd0_sum  = 0;               % component parasite buildup
        CD0_misc = 0;               % misc / stores drag
        CD0_wave = 0;               % wave drag

        %% -------------------------
        % MISC / STORES DRAG
        % --------------------------
        % Base areas [ft^2]
        A_base = [0.136 0.136 1.068];   % [AIM-120, AIM-9X, MK-83] % these do not change and are hardcoded in

        if M > 1
            d_q = (0.064 + 0.042*(M - 3.84)^2) .* A_base;   % equivalent drag area [ft^2]
        else
            d_q = (0.139 + 0.419*(M - 0.161)^2) .* A_base;  % equivalent drag area [ft^2]
        end

        Cd_misc_air_to_air = (6*d_q(1) + 2*d_q(2)) / sref;
        Cd_misc_strike     = (4*d_q(3) + 2*d_q(2)) / sref;

        % Choose the loadout/configuration you want
        CD0_misc = Cd_misc_strike; % strike is the most constraining
        ...
        for i = 1:length(comp)
            ...
            % Reynolds number
            if M == 0
                Re = 1e3;   % avoids log10(0); static case is fake anyway
            else
                Re = rho * V * l_m / mu;
            end
...
mu_ref = 1.716e-5;   % reference viscosity [kg/(m*s)]
