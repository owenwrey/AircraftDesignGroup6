...
for i = 1:length(beta)
    ...
    if (locweight(4,2)-fburn>0)

        weight(4,2) = weight(4,2)-fburn;
        CG(i)=(sum(weight(:,1).*weight(:,2)))./sum(weight(:,2));

    else

        weight(5,2) = weight(5,2) - fburn + weight(4,2);
        weight(4,2) = 0;
        CG(i)=(sum(weight(:,1).*weight(:,2)))./sum(weight(:,2));
...
CG = [26.3203 CG];
miss = 12:-1:1;
plot(CG,miss,"-o",DisplayName="Mission C.G.")
grid on
xlabel("CG location (ft)")
hold on

xline(24.4606,color='r',DisplayName="Forward limit")

Rear = ((0.525*50 + 25.1421-0.3*9.73+0.25*13.63)*122.93 + 592*(25.1421-0.3*9.73+0.25*13.63))/(592 + 122.93);

xline(Rear,color='b',DisplayName="Aft limit")
legend
