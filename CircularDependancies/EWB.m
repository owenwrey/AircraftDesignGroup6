...
weight.total = aircraft.constants.totalWeight;
...
thicknessToChordRatio = aircraft.wing.T2C;
ultimateLoadFactor = aircraft.constants.n_ult;

wing.area = aircraft.wing.area;
wing.AspectRatio = aircraft.wing.AR;
...
aircraft.wing.weight = .036*wing.area^(.758) * weight.fuel^(.0035) * (wing.AspectRatio/(cosd(wing.QuarterChordSweep)^2))^(.6) * cruiseDynamicPressure^(.006)...
    * wing.TaperRatio^(.04) * (100*thicknessToChordRatio/cosd(wing.QuarterChordSweep))^(-.3) * (ultimateLoadFactor*weight.total)^(.49);
