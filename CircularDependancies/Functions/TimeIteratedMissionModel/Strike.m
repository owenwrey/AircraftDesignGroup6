function aircraftResults = Strike(aircraft)
%% fixed engine sizing

% clc
% clearvars -except aircraft Results;
cfg = getConfig(aircraft);
tStart = tic;
displayTable = false;
% segment names
SegNames = {'SW','TKO','CLIMB 1','CR OBD', 'DESC 1', 'COMBAT', 'WP FIRE', 'CLIMB 2', 'CR IBD', ...
'DESC 2', 'LTR2', 'LS'};
% points per segment
npts = [...
3; % 1. start, warmup, taxi (SWT)
3; % 2. takeoff
10; % 3. climb 1
10; % 4. cruise outbound
10; % 5. descent 1
5; % 6. combat
1; % 7. drop/fire ordnance
10; % 8. climb 2
10; % 9. cruise inbound
10; % 10. descend 2
10; % 11. loiter
3; % 12. landing, taxi, shutdown (LTS)
];
npts_sum = sum(npts);
% Tbl = table();
Tbl.SegNum = zeros(npts_sum,1);
Tbl.Seg = repmat({' '},npts_sum,1);
ctr = 1;
for i = 1:length(SegNames)
    Tbl.Seg([ctr:ctr+npts(i)-1]) = SegNames(i);
    Tbl.SegNum([ctr:ctr+npts(i)-1]) = i;
    ctr = ctr + npts(i);
end

f119perf = f119perfGEN;

%% Initialize
Tbl.Time = zeros(npts_sum,1); % time (min)
Tbl.dTime = zeros(npts_sum,1); % delta time (min)
Tbl.Alt = zeros(npts_sum,1); % altitude (ft)
Tbl.rho = zeros(npts_sum,1); % density (slug/ft^3)
Tbl.KEAS = zeros(npts_sum,1); % equivalent airspeed (kt)
Tbl.KTAS = zeros(npts_sum,1); % true airspeed (kt)
Tbl.MACH = zeros(npts_sum,1); % Mach number
Tbl.GS = zeros(npts_sum,1); % ground speed (kt)
Tbl.FPA = zeros(npts_sum,1); % flightpath angle (deg)
Tbl.EnHt = zeros(npts_sum,1); % energy height (ft)
Tbl.Dist = zeros(npts_sum,1); % distance (NM)
Tbl.dDist = zeros(npts_sum,1); % delta distance (NM)
Tbl.dhdt = zeros(npts_sum,1); % rate of climb (ft/min)
Tbl.dVdt = zeros(npts_sum,1); % acceleration
Tbl.Weight = zeros(npts_sum,1); % weight (lb)
Tbl.WtDrop = zeros(npts_sum,1); % dropped weight (lb)
Tbl.WtFrac = zeros(npts_sum,1); % weight fraction
Tbl.CL = zeros(npts_sum,1); % lift coefficient
Tbl.CD0 = zeros(npts_sum,1); % drag polar
Tbl.K1 = zeros(npts_sum,1); % drag polar
Tbl.K2 = zeros(npts_sum,1); % drag polar
Tbl.CDR = zeros(npts_sum,1); % drag polar
Tbl.CD = zeros(npts_sum,1); % drag coefficient
Tbl.L_D = zeros(npts_sum,1); % lift-to-drag ratio
Tbl.Drag = zeros(npts_sum,1); % drag (lbf)
Tbl.Thrust = zeros(npts_sum,1); % thrust (lbf)
Tbl.ThrustLapse = zeros(npts_sum,1); % thrust lapse
Tbl.Ps = zeros(npts_sum,1); % specific excess pwr (ft/min)
Tbl.THROT = zeros(npts_sum,1); % throttle setting
Tbl.FF = zeros(npts_sum,1); % fuel flow (lb/h)
Tbl.dFuel = zeros(npts_sum,1); % delta fuel (lb)
Tbl.FuelRem = zeros(npts_sum,1); % fuel remaining (lb)
Tbl.FuelBurn = zeros(npts_sum,1); % fuel burned (lb)
SFC = zeros(npts_sum,1);

W0 = cfg.W.TOguess;
FuelReq = cfg.W.fuelReq;
tol = cfg.weightTolerance;
diff = 10000; % initialize

%% Loop
while diff > tol

    % if toc(tStart) > 5
    %     error("Strike took too long")
    % end

W_S = cfg.wingLoading; % takeoff wing loading (psf)
%Thrust = cfg.thrust; %lb

% unit conversions
rho_SL = 0.0023769;
mps2kts = 1.94384; % meters per sec to knots
kts2fps = 1/0.59248; % knots to feet per sec
NM2ft = 6067; % nautical miles to feet
ft2m = 0.305; % feet to meters

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% 1. start, warmup
% 3 min duration
% no distance credit

% Segment 1 indices
seg = 1;
istart = sum(npts(1:seg-1)) + 1;
iend = sum(npts(1:seg));

Tbl.Time(istart:iend) = linspace(0,3,npts(seg)); % time (min)
%SFC_idle = cfg.SFC.idle; %lb/(lb.h)
% T_W_idle = cfg.thrustToWeight_idle;

for i = 2:npts(seg)

Tbl.dTime(1) = 0;
Tbl.dTime(i) = Tbl.Time(i) - Tbl.Time(i-1); % delta time (min)
Tbl.Alt(i) = 0; % altitude (ft)
[~, a, ~, rho] = atmosisa(Tbl.Alt(i)*ft2m);
Tbl.rho(1) = rho*0.00194032;
Tbl.rho(i) = rho*0.00194032; % density in slug/ft^3
Tbl.KEAS(i) = 0; % equivalent airspeed (kt)
Tbl.KTAS(i) = 0; % true airspeed (kt)
Tbl.MACH(i) = 0; % Mach number
Tbl.GS(i) = 0; % ground speed (kt)
Tbl.FPA(i) = 0; % flightpath angle (deg)
Tbl.Dist(i) = 0; % distance (NM)1
Tbl.dDist(i) = 0; % delta distance (NM)
Tbl.dhdt(i) = 0; % rate of climb (ft/min)
Tbl.dVdt(i) = 0; % acceleration
Tbl.CL(i) = 0; % lift coefficient
Tbl.CD0(i) =  aircraft.aero.cd0_strike(Tbl.Alt(i),Tbl.MACH(i)); % drag polar
Tbl.K1(i) = 0; % drag polar
Tbl.K2(i) = 0.129; % drag polar
Tbl.CDR(i) = 0; % drag polar
Tbl.CD(i) = 0; % drag coefficient
Tbl.L_D(i) = 0; % lift-to-drag ratio
Tbl.Drag(i) = 0; % drag (lbf)
%[Tbl.Thrust(1), SFC(1)] = f119perf(0,0,false);
Tbl.Thrust(1) = 2*f119perf.mil.fn(0,0);
SFC(1) = f119perf.mil.tsfc(0,0);
%[Tbl.Thrust(i), SFC(i)] = f119perf(0,0,false); % thrust (lbf)
Tbl.Thrust(i) = 2.*f119perf.mil.fn(0,0).*0.6;
SFC(i) = f119perf.mil.tsfc(0,0);
Tbl.ThrustLapse(1) = 1;
Tbl.ThrustLapse(i) = 1; % thrust lapse
Tbl.Ps(i) = 0; % specific excess pwr (ft/min)
Tbl.THROT(i) = 0.6; % throttle setting
Tbl.FF(1) = SFC(1)*Tbl.Thrust(1);
Tbl.FF(i) = SFC(i)*Tbl.Thrust(i); % fuel flow (lb/h)
Tbl.dFuel(i) = (Tbl.FF(i)*Tbl.dTime(i))/60; % delta fuel (lb)
Tbl.FuelBurn(1) = Tbl.dFuel(1);
Tbl.FuelBurn(i) = Tbl.FuelBurn(i-1) + Tbl.dFuel(i); % fuel burned (lb)
Tbl.FuelRem(1) = FuelReq - Tbl.dFuel(1);
Tbl.FuelRem(i) = Tbl.FuelRem(i-1) - Tbl.dFuel(i); % fuel remaining (lb)
Tbl.Weight(1) = W0;
Tbl.Weight(i) = Tbl.Weight(i-1) - Tbl.dFuel(i); % weight (lb)
Tbl.WtDrop(i) = 0; % dropped weight (lb)
Tbl.WtFrac(1) = 1;
Tbl.WtFrac(i) = Tbl.Weight(i)/Tbl.Weight(i-1);% weight fraction
end

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% 2. takeoff
% 1 min duration
% full afterburner (THROT = 1.25)
% no distance credit

% Segment 2 indices
seg = 2;
istart = sum(npts(1:seg-1)) + 1;
iend = sum(npts(1:seg));

% SFC_takeoff = cfg.SFC.takeoff;

Tbl.Time(istart:iend) = linspace(3,4,npts(seg)); % time (min)

for  i = istart:iend

Tbl.dTime(i) = Tbl.Time(i) - Tbl.Time(i-1);% delta time (min)
Tbl.Alt(i) = 0; % altitude (ft)
[~, a, ~, rho] = atmosisa(Tbl.Alt(i)*ft2m);
Tbl.rho(i) = rho*0.00194032; % density in slug/ft^3
Tbl.KEAS(i) = 0; % equivalent airspeed (kt)
Tbl.KTAS(i) = 0; % true airspeed (kt)
Tbl.MACH(i) = 0; % Mach number
Tbl.GS(i) = 0; % ground speed (kt)
Tbl.FPA(i) = 0; % flightpath angle (deg)
Tbl.Dist(i) = 0; % distance (NM)
Tbl.dDist(i) = 0; % delta distance (NM)
Tbl.dhdt(i) = 0; % rate of climb (ft/min)
Tbl.dVdt(i) = 0; % acceleration
Tbl.CL(i) = 0; % lift coefficient
Tbl.CD0(i) =  aircraft.aero.cd0_strike(Tbl.Alt(i),Tbl.MACH(i)); % drag polar  aircraft.aero.cd0_strike(Tbl.Alt(i),Tbl.MACH(i)); % drag polar  aircraft.aero.cd0_strike(Tbl.Alt(i),Tbl.MACH(i)); % drag polar 0.025; % drag polar
Tbl.K1(i) = 0; % drag polar
Tbl.K2(i) = 0.129; % drag polar
Tbl.CDR(i) = 0; % drag polar
Tbl.CD(i) = 0; % drag coefficient
Tbl.L_D(i) = 0; % lift-to-drag ratio
Tbl.Drag(i) = 0; % drag (lbf)
Tbl.ThrustLapse(i) = 1; % thrust lapse
Tbl.Ps(i) = 0; % specific excess pwr (ft/min)
Tbl.THROT(i) = 1.25; % throttle setting
%[Tbl.Thrust(i), SFC(i)] = f119perf(0, 0, true); % thrust (lbf)
Tbl.Thrust(i) = 2.*f119perf.max.fn(0,0);
SFC(i) = f119perf.max.tsfc(0,0);
Tbl.FF(i) = SFC(i)*Tbl.Thrust(i); % fuel flow (lb/h)
Tbl.dFuel(i) = (Tbl.FF(i)*Tbl.dTime(i))/60; % delta fuel (lb)
Tbl.FuelBurn(i) = Tbl.FuelBurn(i-1) + Tbl.dFuel(i); % fuel burned (lb)
Tbl.FuelRem(i) = Tbl.FuelRem(i-1) - Tbl.dFuel(i); % fuel remaining (lb)
Tbl.Weight(i) = Tbl.Weight(i-1) - Tbl.dFuel(i); % weight (lb)
Tbl.WtDrop(i) = 0; % dropped weight (lb)
Tbl.WtFrac(i) = Tbl.Weight(i)/Tbl.Weight(i-1);% weight fraction
end

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%% 3. climb 1
% Mach 0.8
% level off at 30,000 ft
% no afterburner [THROT = 1]
% take distance credit

% Segment 3 indices
seg = 3;
istart = sum(npts(1:seg-1)) + 1;
iend = sum(npts(1:seg));

% SFC_climb = cfg.SFC.climb;

Tbl.Alt(istart:iend) = linspace(0,cfg.cruise.altitude,npts(seg)); % altitude (ft)

for  i = istart:iend

[~, a, ~, rho] = atmosisa(Tbl.Alt(i)*ft2m);
Tbl.rho(i) = rho*0.00194032; % density in slug/ft^3
Tbl.MACH(i) = 0.8; % Mach number
Tbl.KTAS(i) = Tbl.MACH(i)*a*mps2kts; % true airspeed (kt)
Tbl.KEAS(i) = Tbl.KTAS(i)*sqrt(Tbl.rho(i)/rho_SL); % equivalent airspeed (kt)

Tbl.THROT(i) = 0.5; % throttle setting
Tbl.ThrustLapse(i) = 1; % thrust lapse
%[Tbl.Thrust(i), SFC(i)] = f119perf(Tbl.Alt(i), Tbl.MACH(i), false); % thrust (lbf)
Tbl.Thrust(i) = 2.*f119perf.mil.fn(Tbl.Alt(i),Tbl.MACH(i)).*0.5;
SFC(i) = f119perf.mil.tsfc(Tbl.Alt(i), Tbl.MACH(i));
Tbl.FF(i) = SFC(i)*Tbl.Thrust(i); % fuel flow (lb/h)
Tbl.dhdt(i) = ((Tbl.Thrust(i)*Tbl.KTAS(i)*kts2fps*60) - (Tbl.Drag(i)*Tbl.KTAS(i)*kts2fps*60))/Tbl.Weight(i-1) ; % rate of climb (ft/min)
Tbl.Ps(i) = Tbl.dhdt(i); % specific excess pwr (ft/min)

Tbl.CL(i) = (Tbl.WtFrac(i-1)*W_S)/(0.5*Tbl.rho(i)*(Tbl.KTAS(i)*kts2fps)^2); % lift coefficient
Tbl.CD0(i) =  aircraft.aero.cd0_strike(Tbl.Alt(i),Tbl.MACH(i)); % drag polar  aircraft.aero.cd0_strike(Tbl.Alt(i),Tbl.MACH(i)); % drag polar  aircraft.aero.cd0_strike(Tbl.Alt(i),Tbl.MACH(i)); % drag polar 0.025; % drag polar
Tbl.K1(i) = 0; % drag polar
Tbl.K2(i) = 0.129; % drag polar
Tbl.CDR(i) = 0; % drag polar
Tbl.CD(i) = Tbl.CD0(i) + Tbl.K1(i)*Tbl.CL(i) + Tbl.K2(i)*Tbl.CL(i)^2 + Tbl.CDR(i); % drag coefficient
Tbl.L_D(i) = Tbl.CL(i)/Tbl.CD(i); % lift-to-drag ratio
Tbl.Drag(i) = Tbl.CD(i) * 0.5* Tbl.rho(i) * (Tbl.KTAS(i)*kts2fps)^2 * (W0/W_S); % drag (lbf)

Tbl.dTime(i) = (Tbl.Alt(i) - Tbl.Alt(i-1))/Tbl.dhdt(i);% delta time (min)
Tbl.Time(i) = Tbl.dTime(i)+Tbl.Time(i-1); % time (min)
Tbl.FPA(i) = asind(Tbl.dhdt(i)/(Tbl.KTAS(i)*kts2fps*60)); % flightpath angle (deg)
Tbl.GS(i) = Tbl.KTAS(i)*cosd(Tbl.FPA(i)); % ground speed (kt)
Tbl.Dist(istart) = Tbl.Dist(istart - 1);
Tbl.Dist(i) = Tbl.GS(i)*Tbl.dTime(i)/60 + Tbl.Dist(i - 1); % distance (NM)
Tbl.dDist(i) =Tbl.Dist(i) - Tbl.Dist(i-1); % delta distance (NM)
Tbl.dVdt(i) = (Tbl.KTAS(i)*kts2fps - (Tbl.KTAS(i-1)*kts2fps))/(Tbl.dTime(i)*60); % acceleration
Tbl.dVdt(istart) = 0;

Tbl.dFuel(i) = (Tbl.FF(i)*Tbl.dTime(i))/60; % delta fuel (lb)
Tbl.FuelBurn(i) = Tbl.FuelBurn(i-1) + Tbl.dFuel(i); % fuel burned (lb)
Tbl.FuelRem(i) = Tbl.FuelRem(i-1) - Tbl.dFuel(i); % fuel remaining (lb)
Tbl.Weight(i) = Tbl.Weight(i-1) - Tbl.dFuel(i); % weight (lb)
Tbl.WtDrop(i) = 0; % dropped weight (lb)
Tbl.WtFrac(i) = Tbl.Weight(i)/Tbl.Weight(i-1);% weight fraction
end

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%% 4. cruise outbound at 30,000 ft
% 200 NM traveled
% THROT = 1.0
% SFC_cruise = cfg.SFC.cruise; % estimate

% Segment 4 indices
seg = 4;
istart = sum(npts(1:seg-1)) + 1;
iend = sum(npts(1:seg));

Tbl.Dist(istart:iend) = linspace(Tbl.Dist(istart - 1),Tbl.Dist(istart - 1) + 180,npts(seg)); % distance (NM)

for  i = istart:iend

Tbl.dDist(istart) = 0;
Tbl.dDist(i) =Tbl.Dist(i) - Tbl.Dist(i-1); % delta distance (NM)

Tbl.Alt(i) = cfg.cruise.altitude; % Altitude (ft)
[~, a, ~, rho] = atmosisa(Tbl.Alt(i)*ft2m);
Tbl.rho(i) = rho*0.00194032; % density in slug/ft^3
Tbl.MACH(i) = 0.9; % Mach number
Tbl.KTAS(i) = Tbl.MACH(i)*a*mps2kts; % true airspeed (kt)
Tbl.KEAS(i) = Tbl.KTAS(i)*sqrt(Tbl.rho(i)/rho_SL); % equivalent airspeed (kt)

Tbl.CL(i) = (Tbl.WtFrac(i-1)*W_S)/(0.5*Tbl.rho(i)*(Tbl.KTAS(i)*kts2fps)^2); % lift coefficient
Tbl.CD0(i) =  aircraft.aero.cd0_strike(Tbl.Alt(i),Tbl.MACH(i)); % drag polar  
Tbl.K1(i) = 0; % drag polar
Tbl.K2(i) = 0.129; % drag polar
Tbl.CDR(i) = 0; % drag polar
Tbl.CD(i) = Tbl.CD0(i) + Tbl.K1(i)*Tbl.CL(i) + Tbl.K2(i)*Tbl.CL(i)^2 + Tbl.CDR(i); % drag coefficient
Tbl.L_D(i) = Tbl.CL(i)/Tbl.CD(i); % lift-to-drag ratio
Tbl.Drag(i) = Tbl.CD(i) * 0.5* Tbl.rho(i) * (Tbl.KTAS(i)*kts2fps)^2 * (W0/W_S); % drag (lbf)

% solve for throttle setting 
Tbl.ThrustLapse(i) = 1; % thrust lapse
Tbl.Thrust(i) = Tbl.Drag(i); % thrust (lbf)
Tbl.THROT(i) = 0.4; % throttle setting
%[Tbl.Thrust(i), SFC(i)] = f119perf(Tbl.Alt(i), Tbl.MACH(i), false);
Tbl.Thrust(i) = 2.*f119perf.mil.fn(Tbl.Alt(i), Tbl.MACH(i))*0.4;
SFC(i) = f119perf.mil.tsfc(Tbl.Alt(i), Tbl.MACH(i));
Tbl.FF(i) = SFC(i)*Tbl.Thrust(i); % fuel flow (lb/h)
Tbl.dhdt(i) = 0 ; % rate of climb (ft/min)
Tbl.Ps(i) = 0; % specific excess pwr (ft/min)

Tbl.dTime(i) = Tbl.dDist(i)/(Tbl.KTAS(i)/60);% delta time (min)
Tbl.Time(i) = Tbl.dTime(i)+Tbl.Time(i-1); % time (min)
Tbl.FPA(i) = 0; % flightpath angle (deg)
Tbl.GS(i) = Tbl.KTAS(i)*cosd(Tbl.FPA(i)); % ground speed (kt)
Tbl.dVdt(i) = 0; % acceleration

Tbl.dFuel(istart) = 0;
Tbl.dFuel(i) = (Tbl.FF(i)*Tbl.dTime(i))/60; % delta fuel (lb)
Tbl.FuelBurn(i) = Tbl.FuelBurn(i-1) + Tbl.dFuel(i); % fuel burned (lb)
Tbl.FuelRem(i) = Tbl.FuelRem(i-1) - Tbl.dFuel(i); % fuel remaining (lb)
Tbl.Weight(i) = Tbl.Weight(i-1) - Tbl.dFuel(i); % weight (lb)
Tbl.WtDrop(i) = 0; % dropped weight (lb)
Tbl.WtFrac(i) = Tbl.Weight(i)/Tbl.Weight(i-1);% weight fraction

end

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%% 5. descent 1
% 3,000 ft/min

% finish at sea level\

% SFC_descent = cfg.SFC.descent;

% Segment 5 indices
seg = 5;
istart = sum(npts(1:seg-1)) + 1;
iend = sum(npts(1:seg));
Tbl.Alt(istart:iend) = linspace(Tbl.Alt(istart - 1),0,npts(seg)); % altitude (ft)

for  i = istart:iend

[~, a, ~, rho] = atmosisa(Tbl.Alt(i)*ft2m);
Tbl.rho(i) = rho*0.00194032; % density in slug/ft^3
Tbl.MACH(i) = 0.7; % Mach number
Tbl.KTAS(i) = Tbl.MACH(i)*a*mps2kts; % true airspeed (kt)
Tbl.KEAS(i) = Tbl.KTAS(i)*sqrt(Tbl.rho(i)/rho_SL); % equivalent airspeed (kt)

Tbl.THROT(i) = 0.35; % throttle setting
Tbl.ThrustLapse(i) = 1; % thrust lapse
%[Tbl.Thrust(i), SFC(i)] = f119perf(Tbl.Alt(i), Tbl.MACH(i), false); % thrust (lbf)
Tbl.Thrust(i) = 2.*f119perf.mil.fn(Tbl.Alt(i), Tbl.MACH(i))*0.35;
SFC(i) = f119perf.mil.tsfc(Tbl.Alt(i), Tbl.MACH(i));
Tbl.FF(i) = SFC(i)*Tbl.Thrust(i); % fuel flow (lb/h)
Tbl.dhdt(i) = -2000; % rate of climb (ft/min)
Tbl.Ps(i) = Tbl.dhdt(i); % specific excess pwr (ft/min)

Tbl.CL(i) = (Tbl.WtFrac(i-1)*W_S)/(0.5*Tbl.rho(i)*(Tbl.KTAS(i)*kts2fps)^2); % lift coefficient
Tbl.CD0(i) =  aircraft.aero.cd0_strike(Tbl.Alt(i),Tbl.MACH(i)); % drag polar  
Tbl.K1(i) = 0; % drag polar
Tbl.K2(i) = 0.129; % drag polar
Tbl.CDR(i) = 0; % drag polar
Tbl.CD(i) = Tbl.CD0(i) + Tbl.K1(i)*Tbl.CL(i) + Tbl.K2(i)*Tbl.CL(i)^2 + Tbl.CDR(i); % drag coefficient
Tbl.L_D(i) = Tbl.CL(i)/Tbl.CD(i); % lift-to-drag ratio
Tbl.Drag(i) = Tbl.CD(i) * 0.5* Tbl.rho(i) * (Tbl.KTAS(i)*kts2fps)^2 * (W0/W_S); % drag (lbf)

Tbl.dTime(i) = (Tbl.Alt(i) - Tbl.Alt(i-1))/Tbl.dhdt(i);% delta time (min)
Tbl.Time(i) = Tbl.dTime(i)+Tbl.Time(i-1); % time (min)
Tbl.GS(i) = Tbl.KTAS(i)*cosd(Tbl.FPA(i)); % ground speed (kt)
Tbl.FPA(i) = asind(Tbl.dhdt(i)/(Tbl.KTAS(i)*kts2fps*60)); % flightpath angle (deg)
Tbl.dDist(i) = Tbl.GS(i)*Tbl.dTime(i)/60; % delta distance (NM)
Tbl.Dist(i) = Tbl.dDist(i) + Tbl.Dist(i-1); % distance (NM)
Tbl.dVdt(i) = (Tbl.KTAS(i)*kts2fps - (Tbl.KTAS(i-1)*kts2fps))/(Tbl.dTime(i)*60); % acceleration
Tbl.dVdt(istart) = 0;

Tbl.dFuel(i) = (Tbl.FF(i)*Tbl.dTime(i))/60; % delta fuel (lb)
Tbl.FuelBurn(i) = Tbl.FuelBurn(i-1) + Tbl.dFuel(i); % fuel burned (lb)
Tbl.FuelRem(i) = Tbl.FuelRem(i-1) - Tbl.dFuel(i); % fuel remaining (lb)
Tbl.Weight(i) = Tbl.Weight(i-1) - Tbl.dFuel(i); % weight (lb)
Tbl.WtDrop(i) = 0; % dropped weight (lb)
Tbl.WtFrac(i) = Tbl.Weight(i)/Tbl.Weight(i-1);% weight fraction

end

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%% 6. combat at sea level
% 5 min duration
% COMBAT-LOADED 15000 pounds of THRUST
% THROT = 1.25
% no distance credit
% SFC_combat = cfg.SFC.combat;

% Segment 6 indices
seg = 6;
istart = sum(npts(1:seg-1)) + 1;
iend = sum(npts(1:seg));

Tbl.Time(istart:iend) = linspace((Tbl.Time(istart-1)),(Tbl.Time(istart-1)+5),npts(seg)); % distance (NM)

for  i = istart:iend

Tbl.dTime(istart) = 0; 
Tbl.dTime(i) = Tbl.Time(i) - Tbl.Time(i-1); % delta distance (NM)
Tbl.Dist(i) = Tbl.Dist(istart-1);
Tbl.dDist(i) = 0;

Tbl.Alt(i) = 0; % Altitude (ft)
[~, a, ~, rho] = atmosisa(Tbl.Alt(i)*ft2m);
Tbl.rho(i) = rho*0.00194032; % density in slug/ft^3
Tbl.KTAS(i) = 595; % true airspeed (kt)
Tbl.KEAS(i) = Tbl.KTAS(i)*sqrt(Tbl.rho(i)/rho_SL); % equivalent airspeed (kt)
Tbl.MACH(i) = Tbl.KTAS(i)/(a*mps2kts); % Mach number

Tbl.CL(i) = (Tbl.WtFrac(i-1)*W_S)/(0.5*Tbl.rho(i)*(Tbl.KTAS(i)*kts2fps)^2); % lift coefficient
Tbl.CD0(i) =  aircraft.aero.cd0_strike(Tbl.Alt(i),Tbl.MACH(i)); % drag polar  aircraft.aero.cd0_strike(Tbl.Alt(i),Tbl.MACH(i)); % drag polar  aircraft.aero.cd0_strike(Tbl.Alt(i),Tbl.MACH(i)); % drag polar 0.025; % drag polar
Tbl.K1(i) = 0; % drag polar
Tbl.K2(i) = 0.129; % drag polar
Tbl.CDR(i) = 0; % drag polar
Tbl.CD(i) = Tbl.CD0(i) + Tbl.K1(i)*Tbl.CL(i) + Tbl.K2(i)*Tbl.CL(i)^2 + Tbl.CDR(i); % drag coefficient
Tbl.L_D(i) = Tbl.CL(i)/Tbl.CD(i); % lift-to-drag ratio
Tbl.Drag(i) = Tbl.CD(i) * 0.5* Tbl.rho(i) * (Tbl.KTAS(i)*kts2fps)^2 * (W0/W_S); % drag (lbf)

% solve for throttle setting 
Tbl.THROT(i) = 1.25; % throttle setting
Tbl.ThrustLapse(i) = 1; % thrust laps
%[Tbl.Thrust(i), SFC(i)] = f119perf(Tbl.Alt(i), Tbl.MACH(i), true); % thrust (lbf)
Tbl.Thrust(i) = 2.*f119perf.max.fn(Tbl.Alt(i), Tbl.MACH(i)).*0.6;
SFC(i) = f119perf.max.tsfc(Tbl.Alt(i), Tbl.MACH(i));
Tbl.FF(i) = SFC(i)*Tbl.Thrust(i); % fuel flow (lb/h)
Tbl.dhdt(i) = 0 ; % rate of climb (ft/min)
Tbl.Ps(i) = 0; % specific excess pwr (ft/min)

Tbl.FPA(i) = 0; % flightpath angle (deg)
Tbl.GS(i) = Tbl.KTAS(i)*cosd(Tbl.FPA(i)); % ground speed (kt)
Tbl.dVdt(i) = 0; % acceleration

Tbl.dFuel(istart) = 0;
Tbl.dFuel(i) = (Tbl.FF(i)*Tbl.dTime(i))/60; % delta fuel (lb)
Tbl.FuelBurn(i) = Tbl.FuelBurn(i-1) + Tbl.dFuel(i); % fuel burned (lb)
Tbl.FuelRem(i) = Tbl.FuelRem(i-1) - Tbl.dFuel(i); % fuel remaining (lb)
Tbl.Weight(i) = Tbl.Weight(i-1) - Tbl.dFuel(i); % weight (lb)
Tbl.WtDrop(i) = 0; % dropped weight (lb)
Tbl.WtFrac(i) = Tbl.Weight(i)/Tbl.Weight(i-1);% weight fraction
end

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% 7. fire ordnance at sea level
% no time elapsed
% no distance credit
% Segment 7 indices
% SFC_weightdrop = cfg.SFC.weightdrop;

seg = 7;
istart = sum(npts(1:seg-1)) + 1;
iend = sum(npts(1:seg));

for  i = istart:iend

Tbl.Time(i) = Tbl.Time(istart - 1);
Tbl.dTime(i) = 0; % delta time (min)
Tbl.Alt(i) = 0; % altitude (ft)
[~, a, ~, rho] = atmosisa(Tbl.Alt(i)*ft2m);
Tbl.rho(i) = rho*0.00194032; % density in slug/ft^3
Tbl.KEAS(i) = 595; % equivalent airspeed (kt)
Tbl.KTAS(i) = 595; % true airspeed (kt)
Tbl.MACH(i) = 0; % Mach number
Tbl.GS(i) = 0; % ground speed (kt)
Tbl.FPA(i) = 0; % flightpath angle (deg)
Tbl.Dist(i) = Tbl.Dist(istart - 1); % distance (NM)
Tbl.dDist(i) = 0; % delta distance (NM)
Tbl.dhdt(i) = 0; % rate of climb (ft/min)
Tbl.dVdt(i) = 0; % acceleration
Tbl.CL(i) = 0; % lift coefficient
Tbl.CD0(i) =  aircraft.aero.cd0_strike(Tbl.Alt(i),Tbl.MACH(i)); % drag polar 
Tbl.K1(i) = 0; % drag polar
Tbl.K2(i) = 0.129; % drag polar
Tbl.CDR(i) = 0; % drag polar
Tbl.CD(i) = 0; % drag coefficient
Tbl.L_D(i) = 0; % lift-to-drag ratio
Tbl.Drag(i) = 0; % drag (lbf)
Tbl.ThrustLapse(i) = 1; % thrust lapse
Tbl.Ps(i) = 0; % specific excess pwr (ft/min)
Tbl.THROT(i) = 0; % throttle setting
%[Tbl.Thrust(i), SFC(i)] = f119perf(Tbl.Alt(i), Tbl.MACH(i), true); % thrust (lbf)
Tbl.Thrust(i) = 2.*f119perf.max.fn(Tbl.Alt(i), Tbl.MACH(i)).*0.7;
SFC(i) = f119perf.max.tsfc(Tbl.Alt(i), Tbl.MACH(i));
Tbl.FF(i) = 0; % fuel flow (lb/h)
Tbl.dFuel(i) = 0; % delta fuel (lb)
Tbl.FuelBurn(i) = Tbl.FuelBurn(i-1) + Tbl.dFuel(i); % fuel burned (lb)
Tbl.FuelRem(i) = Tbl.FuelRem(istart - 1); % fuel remaining (lb)
Tbl.Weight(i) = Tbl.Weight(istart - 1); % weight (lb)
Tbl.WtDrop(i) = 0; % dropped weight (lb)
Tbl.WtFrac(i) = Tbl.Weight(i)/Tbl.Weight(istart - 1);% weight fraction
end

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%% 8. climb 2 - no afterburner
% Mach 0.8
% level off at 28,000 ft
% no afterburner [THROT = 1]
% take distance credit

% SFC_climb2 = cfg.SFC.climb;

% Segment 8 indices
seg = 8;
istart = sum(npts(1:seg-1)) + 1;
iend = sum(npts(1:seg));

Tbl.Alt(istart:iend) = linspace(0,cfg.cruise.altitude,npts(seg)); % altitude (ft)

for  i = istart:iend

[~, a, ~, rho] = atmosisa(Tbl.Alt(i)*ft2m);
Tbl.rho(i) = rho*0.00194032; % density in slug/ft^3
Tbl.MACH(i) = 0.9; % Mach number
Tbl.KTAS(i) = Tbl.MACH(i)*a*mps2kts; % true airspeed (kt)
Tbl.KEAS(i) = Tbl.KTAS(i)*sqrt(Tbl.rho(i)/rho_SL); % equivalent airspeed (kt)

Tbl.THROT(i) = 0.45; % throttle setting
Tbl.ThrustLapse(i) = 1; % thrust laps
%[Tbl.Thrust(i), SFC(i)] = f119perf(Tbl.Alt(i), Tbl.MACH(i), false); % thrust (lbf)
Tbl.Thrust(i) = 2.*f119perf.mil.fn(Tbl.Alt(i), Tbl.MACH(i)).*0.7;
SFC(i) = f119perf.mil.tsfc(Tbl.Alt(i), Tbl.MACH(i));
Tbl.FF(i) = SFC(i)*Tbl.Thrust(i); % fuel flow (lb/h)
Tbl.dhdt(i) = ((Tbl.Thrust(i)*Tbl.KTAS(i)*kts2fps*60) - (Tbl.Drag(i)*Tbl.KTAS(i)*kts2fps*60))/Tbl.Weight(i-1) ; % rate of climb (ft/min)
Tbl.Ps(i) = Tbl.dhdt(i); % specific excess pwr (ft/min)

Tbl.CL(i) = (Tbl.WtFrac(i-1)*W_S)/(0.5*Tbl.rho(i)*(Tbl.KTAS(i)*kts2fps)^2); % lift coefficient
Tbl.CD0(i) =  aircraft.aero.cd0_strike(Tbl.Alt(i),Tbl.MACH(i)); % drag polar  
Tbl.K1(i) = 0; % drag polar
Tbl.K2(i) = 0.129; % drag polar
Tbl.CDR(i) = 0; % drag polar
Tbl.CD(i) = Tbl.CD0(i) + Tbl.K1(i)*Tbl.CL(i) + Tbl.K2(i)*Tbl.CL(i)^2 + Tbl.CDR(i); % drag coefficient
Tbl.L_D(i) = Tbl.CL(i)/Tbl.CD(i); % lift-to-drag ratio
Tbl.Drag(i) = Tbl.CD(i) * 0.5* Tbl.rho(i) * (Tbl.KTAS(i)*kts2fps)^2 * (W0/W_S); % drag (lbf)

Tbl.dTime(i) = (Tbl.Alt(i) - Tbl.Alt(i-1))/Tbl.dhdt(i);% delta time (min)
Tbl.Time(i) = Tbl.dTime(i)+Tbl.Time(i-1); % time (min)
Tbl.FPA(i) = asind(Tbl.dhdt(i)/(Tbl.KTAS(i)*kts2fps*60)); % flightpath angle (deg)
Tbl.GS(i) = Tbl.KTAS(i)*cosd(Tbl.FPA(i)); % ground speed (kt)
Tbl.Dist(istart) = Tbl.Dist(istart - 1);
Tbl.Dist(i) = Tbl.GS(i)*Tbl.dTime(i)/60 + Tbl.Dist(i - 1); % distance (NM)
Tbl.dDist(i) =Tbl.Dist(i) - Tbl.Dist(i-1); % delta distance (NM)
Tbl.dVdt(i) = (Tbl.KTAS(i)*kts2fps - (Tbl.KTAS(i-1)*kts2fps))/(Tbl.dTime(i)*60); % acceleration
Tbl.dVdt(istart) = 0;

Tbl.dFuel(i) = (Tbl.FF(i)*Tbl.dTime(i))/60; % delta fuel (lb)
Tbl.FuelBurn(i) = Tbl.FuelBurn(i-1) + Tbl.dFuel(i); % fuel burned (lb)
Tbl.FuelRem(i) = Tbl.FuelRem(i-1) - Tbl.dFuel(i); % fuel remaining (lb)
Tbl.Weight(i) = Tbl.Weight(i-1) - Tbl.dFuel(i); % weight (lb)
Tbl.WtDrop(i) = 0; % dropped weight (lb)
Tbl.WtFrac(i) = Tbl.Weight(i)/Tbl.Weight(i-1);% weight fraction
end

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%% 9. cruise inbound at 30,000 ft
% 200 NM traveled
% THROT = 1.0

% SFC_cruise2 = cfg.SFC.cruise;

% Segment 9 indices
seg = 9;
istart = sum(npts(1:seg-1)) + 1;
iend = sum(npts(1:seg));

Tbl.Dist(istart:iend) = linspace(Tbl.Dist(istart - 1), (cfg.cruise.distance.out.STK + cfg.cruise.distance.in.STK-200),npts(seg)); % distance (NM)

for  i = istart:iend

Tbl.dDist(istart) = 0;
Tbl.dDist(i) =Tbl.Dist(i) - Tbl.Dist(i-1); % delta distance (NM)

Tbl.Alt(i) = cfg.cruise.altitude; % altitude (ft)
[~, a, ~, rho] = atmosisa(Tbl.Alt(i)*ft2m);
Tbl.rho(i) = rho*0.00194032; % density in slug/ft^3
Tbl.KTAS(i) = 530; % true airspeed (kt)
Tbl.KEAS(i) = Tbl.KTAS(i)*sqrt(Tbl.rho(i)/rho_SL); % equivalent airspeed (kt)
Tbl.MACH(i) = Tbl.KTAS(i)/(a*mps2kts); % Mach number

Tbl.CL(i) = (Tbl.WtFrac(i-1)*W_S)/(0.5*Tbl.rho(i)*(Tbl.KTAS(i)*kts2fps)^2); % lift coefficient
Tbl.CD0(i) =  aircraft.aero.cd0_strike(Tbl.Alt(i),Tbl.MACH(i)); % drag polar  aircraft.aero.cd0_strike(Tbl.Alt(i),Tbl.MACH(i)); % drag polar  aircraft.aero.cd0_strike(Tbl.Alt(i),Tbl.MACH(i)); % drag polar 0.025; % drag polar
Tbl.K1(i) = 0; % drag polar
Tbl.K2(i) = 0.129; % drag polar
Tbl.CDR(i) = 0; % drag polar
Tbl.CD(i) = Tbl.CD0(i) + Tbl.K1(i)*Tbl.CL(i) + Tbl.K2(i)*Tbl.CL(i)^2 + Tbl.CDR(i); % drag coefficient
Tbl.L_D(i) = Tbl.CL(i)/Tbl.CD(i); % lift-to-drag ratio
Tbl.Drag(i) = Tbl.CD(i) * 0.5* Tbl.rho(i) * (Tbl.KTAS(i)*kts2fps)^2 * (W0/W_S); % drag (lbf)

% solve for throttle setting 
Tbl.Thrust(i) = Tbl.Drag(i); % thrust (lbf)
Tbl.ThrustLapse(i) = 1; % thrust laps
Tbl.THROT(i) = 0.35 ; % throttle setting
%[Tbl.Thrust(i), SFC(i)] = f119perf(Tbl.Alt(i), Tbl.MACH(i), false);
Tbl.Thrust(i) = 2.*f119perf.mil.fn(Tbl.Alt(i), Tbl.MACH(i)).*0.4;
SFC(i) = f119perf.mil.tsfc(Tbl.Alt(i), Tbl.MACH(i));
Tbl.FF(i) = SFC(i)*Tbl.Thrust(i); % fuel flow (lb/h)
Tbl.dhdt(i) = 0 ; % rate of climb (ft/min)
Tbl.Ps(i) = 0; % specific excess pwr (ft/min)

Tbl.dTime(i) = Tbl.dDist(i)/(Tbl.KTAS(i)/60);% delta time (min)
Tbl.Time(i) = Tbl.dTime(i)+Tbl.Time(i-1); % time (min)
Tbl.FPA(i) = 0; % flightpath angle (deg)
Tbl.GS(i) = Tbl.KTAS(i)*cosd(Tbl.FPA(i)); % ground speed (kt)
Tbl.dVdt(i) = 0; % acceleration

Tbl.dFuel(istart) = 0;
Tbl.dFuel(i) = (Tbl.FF(i)*Tbl.dTime(i))/60; % delta fuel (lb)
Tbl.FuelBurn(i) = Tbl.FuelBurn(i-1) + Tbl.dFuel(i); % fuel burned (lb)
Tbl.FuelRem(i) = Tbl.FuelRem(i-1) - Tbl.dFuel(i); % fuel remaining (lb)
Tbl.Weight(i) = Tbl.Weight(i-1) - Tbl.dFuel(i); % weight (lb)
Tbl.WtDrop(i) = 0; % dropped weight (lb)
Tbl.WtFrac(i) = Tbl.Weight(i)/Tbl.Weight(i-1);% weight fraction
end

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%% 10. descent 2
% 3,000 ft/min

% finish at sea level
% SFC_descent = cfg.SFC.descent;

% Segment 10 indices
seg = 10;
istart = sum(npts(1:seg-1)) + 1;
iend = sum(npts(1:seg));

Tbl.Alt(istart:iend) = linspace(Tbl.Alt(istart - 1),0,npts(seg)); % altitude (ft)

for  i = istart:iend

[~, a, ~, rho] = atmosisa(Tbl.Alt(i)*ft2m);
Tbl.rho(i) = rho*0.00194032; % density in slug/ft^3
Tbl.MACH(i) = 0.7; % Mach number
Tbl.KTAS(i) = Tbl.MACH(i)*a*mps2kts; % true airspeed (kt)
Tbl.KEAS(i) = Tbl.KTAS(i)*sqrt(Tbl.rho(i)/rho_SL); % equivalent airspeed (kt)

Tbl.THROT(i) = 0.3; % throttle setting
Tbl.ThrustLapse(i) = 1; % thrust laps
%[Tbl.Thrust(i), SFC(i)] = f119perf(Tbl.Alt(i), Tbl.MACH(i), false); % thrust (lbf)
Tbl.Thrust(i) = 2.*f119perf.mil.fn(Tbl.Alt(i), Tbl.MACH(i))*0.3;
SFC(i) = f119perf.mil.tsfc(Tbl.Alt(i), Tbl.MACH(i));
Tbl.FF(i) = SFC(i)*Tbl.Thrust(i); % fuel flow (lb/h)
Tbl.dhdt(i) = -2000; % rate of climb (ft/min)
Tbl.Ps(i) = Tbl.dhdt(i); % specific excess pwr (ft/min)

Tbl.CL(i) = (Tbl.WtFrac(i-1)*W_S)/(0.5*Tbl.rho(i)*(Tbl.KTAS(i)*kts2fps)^2); % lift coefficient
Tbl.CD0(i) =  aircraft.aero.cd0_strike(Tbl.Alt(i),Tbl.MACH(i)); % drag polar 
Tbl.K1(i) = 0; % drag polar
Tbl.K2(i) = 0.129; % drag polar
Tbl.CDR(i) = 0; % drag polar
Tbl.CD(i) = Tbl.CD0(i) + Tbl.K1(i)*Tbl.CL(i) + Tbl.K2(i)*Tbl.CL(i)^2 + Tbl.CDR(i); % drag coefficient
Tbl.L_D(i) = Tbl.CL(i)/Tbl.CD(i); % lift-to-drag ratio
Tbl.Drag(i) = Tbl.CD(i) * 0.5* Tbl.rho(i) * (Tbl.KTAS(i)*kts2fps)^2 * (W0/W_S); % drag (lbf)

Tbl.dTime(i) = (Tbl.Alt(i) - Tbl.Alt(i-1))/Tbl.dhdt(i);% delta time (min)
Tbl.Time(i) = Tbl.dTime(i)+Tbl.Time(i-1); % time (min)
Tbl.GS(i) = Tbl.KTAS(i)*cosd(Tbl.FPA(i)); % ground speed (kt)
Tbl.FPA(i) = asind(Tbl.dhdt(i)/(Tbl.KTAS(i)*kts2fps*60)); % flightpath angle (deg)
Tbl.dDist(i) = Tbl.GS(i)*Tbl.dTime(i)/60; % delta distance (NM)
Tbl.Dist(i) = Tbl.dDist(i) + Tbl.Dist(i-1); % distance (NM)
Tbl.dVdt(i) = (Tbl.KTAS(i)*kts2fps - (Tbl.KTAS(i-1)*kts2fps))/(Tbl.dTime(i)*60); % acceleration
Tbl.dVdt(istart) = 0;

Tbl.dFuel(i) = (Tbl.FF(i)*Tbl.dTime(i))/60; % delta fuel (lb)
Tbl.FuelBurn(i) = Tbl.FuelBurn(i-1) + Tbl.dFuel(i); % fuel burned (lb)
Tbl.FuelRem(i) = Tbl.FuelRem(i-1) - Tbl.dFuel(i); % fuel remaining (lb)
Tbl.Weight(i) = Tbl.Weight(i-1) - Tbl.dFuel(i); % weight (lb)
Tbl.WtDrop(i) = 0; % dropped weight (lb)
Tbl.WtFrac(i) = Tbl.Weight(i)/Tbl.Weight(i-1);% weight fraction

end

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%% 11. loiter at sea level
% 20 min duration
% no distance credit
% SFC_loiter = cfg.SFC.loiter;

% Segment 11 indices
seg = 11;
istart = sum(npts(1:seg-1)) + 1;
iend = sum(npts(1:seg));

Tbl.Time(istart:iend) = linspace((Tbl.Time(istart-1)),(Tbl.Time(istart-1)+20),npts(seg)); % distance (NM)

for  i = istart:iend

Tbl.dTime(istart) = 0; 
Tbl.dTime(i) = Tbl.Time(i) - Tbl.Time(i-1); % delta distance (NM)
Tbl.Dist(i) = Tbl.Dist(istart-1);
Tbl.dDist(i) = 0;

Tbl.Alt(i) = 0; % Altitude (ft)
[~, a, ~, rho] = atmosisa(Tbl.Alt(i)*ft2m);
Tbl.rho(i) = rho*0.00194032; % density in slug/ft^3
Tbl.KTAS(i) = 212; % true airspeed (kt)
Tbl.KEAS(i) = Tbl.KTAS(i)*sqrt(Tbl.rho(i)/rho_SL); % equivalent airspeed (kt)
Tbl.MACH(i) = Tbl.KTAS(i)/(a*mps2kts); % Mach number

Tbl.CL(i) = (Tbl.WtFrac(i-1)*W_S)/(0.5*Tbl.rho(i)*(Tbl.KTAS(i)*kts2fps)^2); % lift coefficient
Tbl.CD0(i) =  aircraft.aero.cd0_strike(Tbl.Alt(i),Tbl.MACH(i)); % drag polar  aircraft.aero.cd0_strike(Tbl.Alt(i),Tbl.MACH(i)); % drag polar  aircraft.aero.cd0_strike(Tbl.Alt(i),Tbl.MACH(i)); % drag polar 0.025; % drag polar
Tbl.K1(i) = 0; % drag polar
Tbl.K2(i) = 0.129; % drag polar
Tbl.CDR(i) = 0; % drag polar
Tbl.CD(i) = Tbl.CD0(i) + Tbl.K1(i)*Tbl.CL(i) + Tbl.K2(i)*Tbl.CL(i)^2 + Tbl.CDR(i); % drag coefficient
Tbl.L_D(i) = Tbl.CL(i)/Tbl.CD(i); % lift-to-drag ratio
Tbl.Drag(i) = Tbl.CD(i) * 0.5* Tbl.rho(i) * (Tbl.KTAS(i)*kts2fps)^2 * (W0/W_S); % drag (lbf)

% solve for throttle setting 
Tbl.Thrust(i) = Tbl.Drag(i); % thrust (lbf)
Tbl.ThrustLapse(i) = 1; % thrust laps
Tbl.THROT(i) = 0.2; % throttle setting
%[Tbl.Thrust(i), SFC(i)] = f119perf(Tbl.Alt(i), Tbl.MACH(i), false);
Tbl.Thrust(i) = 2.*f119perf.mil.fn(Tbl.Alt(i), Tbl.MACH(i))*0.2;
SFC(i) = f119perf.mil.tsfc(Tbl.Alt(i), Tbl.MACH(i));
Tbl.FF(i) = SFC(i)*Tbl.Thrust(i); % fuel flow (lb/h)
Tbl.dhdt(i) = 0 ; % rate of climb (ft/min)
Tbl.Ps(i) = 0; % specific excess pwr (ft/min)

Tbl.FPA(i) = 0; % flightpath angle (deg)
Tbl.GS(i) = Tbl.KTAS(i)*cosd(Tbl.FPA(i)); % ground speed (kt)
Tbl.dVdt(i) = 0; % acceleration

Tbl.dFuel(istart) = 0;
Tbl.dFuel(i) = (Tbl.FF(i)*Tbl.dTime(i))/60; % delta fuel (lb)
Tbl.FuelBurn(i) = Tbl.FuelBurn(i-1) + Tbl.dFuel(i); % fuel burned (lb)
Tbl.FuelRem(i) = Tbl.FuelRem(i-1) - Tbl.dFuel(i); % fuel remaining (lb)
Tbl.Weight(i) = Tbl.Weight(i-1) - Tbl.dFuel(i); % weight (lb)
Tbl.WtDrop(i) = 0; % dropped weight (lb)
Tbl.WtFrac(i) = Tbl.Weight(i)/Tbl.Weight(i-1);% weight fraction
end

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%% 12. landing, taxi-in, shutdown
% 3 min duration

% (T/W)_idle = 0.05;
% no distance credit
% SFC_shutdown = cfg.SFC.shutdown;

% Segment 12 indices
seg = 12;
istart = sum(npts(1:seg-1)) + 1;
iend = sum(npts(1:seg));

for  i = istart:iend

Tbl.Time(istart:iend) = linspace((Tbl.Time(istart-1)),(Tbl.Time(istart-1)+ 3),npts(seg)); % time (min)
Tbl.dTime(istart) = 0;
Tbl.dTime(i) = Tbl.Time(i) - Tbl.Time(i-1);% delta time (min)
Tbl.Alt(i) = 0; % altitude (ft)
[~, a, ~, rho] = atmosisa(Tbl.Alt(i)*ft2m);
Tbl.rho(i) = rho*0.00194032; % density in slug/ft^3
Tbl.KEAS(i) = 0; % equivalent airspeed (kt)
Tbl.KTAS(i) = 0; % true airspeed (kt)
Tbl.MACH(i) = 0; % Mach number
Tbl.GS(i) = 0; % ground speed (kt)
Tbl.FPA(i) = 0; % flightpath angle (deg)
Tbl.Dist(i) = Tbl.Dist(istart-1); % distance (NM)
Tbl.dDist(i) = 0; % delta distance (NM)
Tbl.dhdt(i) = 0; % rate of climb (ft/min)
Tbl.dVdt(i) = 0; % acceleration
Tbl.CL(i) = 0; % lift coefficient
Tbl.CD0(i) =  aircraft.aero.cd0_strike(Tbl.Alt(i),Tbl.MACH(i)); % drag polar  aircraft.aero.cd0_strike(Tbl.Alt(i),Tbl.MACH(i)); % drag polar  aircraft.aero.cd0_strike(Tbl.Alt(i),Tbl.MACH(i)); % drag polar 0.025; % drag polar
Tbl.K1(i) = 0; % drag polar
Tbl.K2(i) = 0.129; % drag polar
Tbl.CDR(i) = 0; % drag polar
Tbl.CD(i) = 0; % drag coefficient
Tbl.L_D(i) = 0; % lift-to-drag ratio
Tbl.Drag(i) = 0; % drag (lbf)
Tbl.ThrustLapse(i) = 1; % thrust lapse
Tbl.Ps(i) = 0; % specific excess pwr (ft/min)
Tbl.THROT(i) = 0.3; % throttle setting
%[Tbl.Thrust(i), SFC(i)] = f119perf(Tbl.Alt(i), Tbl.MACH(i), false); % thrust (lbf)
Tbl.Thrust(i) = 2.*f119perf.mil.fn(Tbl.Alt(i), Tbl.MACH(i)).*0.3;
SFC(i) = f119perf.mil.tsfc(Tbl.Alt(i), Tbl.MACH(i));
Tbl.FF(i) = SFC(i)*Tbl.Thrust(i); % fuel flow (lb/h)
Tbl.dFuel(istart) = 0;
Tbl.dFuel(i) = (Tbl.FF(i)*Tbl.dTime(i))/60; % delta fuel (lb)F
Tbl.FuelBurn(i) = Tbl.FuelBurn(i-1) + Tbl.dFuel(i); % fuel burned (lb)
Tbl.FuelRem(istart) = Tbl.FuelRem(istart - 1);
Tbl.FuelRem(i) = Tbl.FuelRem(i-1) - Tbl.dFuel(i); % fuel remaining (lb)
Tbl.Weight(i) = Tbl.Weight(i-1) - Tbl.dFuel(i); % weight (lb)
Tbl.WtDrop(i) = 0; % dropped weight (lb)
Tbl.WtFrac(i) = Tbl.Weight(i)/Tbl.Weight(i-1);% weight fraction
end




% Any missing info: grab from Exam 2 problem setup
% getting atmosphere properties
% [T_K, a_ms, P_Pa, rho_kgm3] = atmosisa(Alt_m)
% thrust model
% THRUST = [SLS THRUST] * [LAPSE] * [THROT]
% THROT: [0, 1.25]
% 0-1.00: no afterburner
% 1.00 - 1.25: afterburner engaged
% thrust lapse model


% equations

Tbl.EnHt = Tbl.Alt + (Tbl.KTAS*NM2ft/60).^2/(2*32.17);
% Tbl.GS = Tbl.KTAS.*cosd(Tbl.FPA);

if displayTable == true
    disp(Tbl);
end

W_crew = cfg.W.crew;
W_payload = cfg.W.PL.A2A; % weight of weapons

if isfield(aircraft, "weight") && isfield(aircraft.weight, "empty")
    noEmptyWeightFlag = false;
    OEW = aircraft.weight.empty;
    
else
    noEmptyWeightFlag = true;
    A = 2.34;
    C = -0.13;
    EWF = A*W0^C;
    OEW = EWF*W0;
end

FuelAllow = cfg.fuelBufferPercent*(Tbl.FuelBurn(iend)); % 6% fuel allowance 
FuelReq = FuelAllow + Tbl.FuelBurn(iend); 
FuelAvail = W0 - OEW - W_crew - W_payload;
FuelExcess = FuelAvail - FuelReq;
diff = abs(FuelExcess);


W0_calc = FuelReq + OEW + W_crew + W_payload;

W0 = W0_calc;   

if abs(imag(W0)) > 1e-6
    W0 = real(W0);
    diff = tol + 1;
    continue;
end

if noEmptyWeightFlag == true
    warning('No aircraft empty weight provided, using A*W0^C')
end

end % while

%% Assign results to aircraft struct
aircraft.weight.total = W0;
aircraft.weight.fuel = FuelAvail;
aircraft.weight.empty = OEW;
aircraft.weight.totalOnLanding = Tbl.Weight(end);

%% Print and Plot
% figure
% plot(Tbl.Time, Tbl.FuelBurn)
% xlabel("Time (min)")
% ylabel("Total Fuel Burn (lb)")
% 
% fprintf('Strike Mission Results\n')
% fprintf('Converged Gross Weight is %5.0f lbs\n', W0)
% fprintf('Fuel Required is %5.0f lbs', FuelReq)

Tbl = struct2table(Tbl);
aircraftResults = aircraft;
aircraftResults.TimeStepTable = Tbl;
end